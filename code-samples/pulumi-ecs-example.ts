...
const instancesSecurityGroup = new awsx.ec2.SecurityGroup('dev-instances-sg', {
  vpc,
...
const userData: awsx.autoscaling.AutoScalingUserData = {
  extraBootcmdLines: () =>
    clusterIdentifier.apply((clusterId) => [
...
// Creates our ASG
const asg = new aws.autoscaling.Group('dev-asg', {
  vpc,
  // Defines the min and max number os instances allowed on this ASG
  templateParameters: { minSize: 1, maxSize: 1 },
...
const capacityProvider = new aws.ecs.CapacityProvider('dev-capacity', {
...
asg.scaleToTrackMetric('scale-memory-reserv', {
